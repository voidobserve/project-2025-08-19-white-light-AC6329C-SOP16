...
static int att_write_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    case ATT_CHARACTERISTIC_fff1_01_VALUE_HANDLE:
        ...
        if(buffer[0] == 0x01 && buffer[1] == 0x03)  //与APP同步数据
        {
            ...
            ver_buff[1] = 0x05;
            memcpy(&ver_buff[2],"guan_ZY001_0.0.1", 16);
            app_send_user_data(connection_handle,ATT_CHARACTERISTIC_fff1_01_VALUE_HANDLE,ver_buff,18,ATT_OP_AUTO_READ_CCC);
            ...
            result = app_send_user_data(connection_handle, ATT_CHARACTERISTIC_fff1_01_VALUE_HANDLE, Send_buffer, 9, ATT_OP_AUTO_READ_CCC);
            // printf("result = %d", result);
            printf_buf(Ble_Addr,6);
            os_time_dly(1);
            //-------------流星速度-----------------------------------
            Send_buffer[6] = 0x2F;
            Send_buffer[7] = 0x01;
...
void zd_fb_2_app(u8 *p, u8 len)
{
    ...
    app_send_user_data(fd_handle, ATT_CHARACTERISTIC_fff1_01_VALUE_HANDLE, Send_buffer, len+6, ATT_OP_AUTO_READ_CCC);
}
