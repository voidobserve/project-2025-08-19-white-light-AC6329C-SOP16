...
const struct task_info task_info_table[] = {
    ...
    {"msg_task", 3, 128, 128},  // 用户消息处理线程
    {"user_task", 3, 256, 256}, // 用户自定义的线程
...
void led_main(void)
{
    ...
    /*
        注意，要先创建接收消息的线程，再跑主函数
    */
    extern void user_msg_handle_task(void *p);
    task_create(user_msg_handle_task, NULL, "msg_task");

    sys_timer_add(NULL, main_while, 1); // 1ms调用

    extern void user_task(void *p);
    task_create(user_task, NULL, "user_task");
}
/****************************************************************************************
**名称:输出显示
